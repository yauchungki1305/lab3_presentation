...
author: "Claire Saunders, Harry Yau, Justin Mak, Sasha Sah"
...
https://github.ubc.ca/mds-2025-26/DSCI_521_lab3_group08
...
``` {r question_4, error = TRUE}
prin("hello world")

# Prints hello world. But has an error. Chunk option error = TRUE, so the file will continue to knit
```
...
The code chuck below pass through two options echo = FALSE and eval =
FALSE. The echo option is to print the code of the chuck and the eval is
to print the output of the chuck. As I have set both the options FALSE,
therefore, nothing about the code chuck (code and output) will shows
after export.

```{r question_5, echo = FALSE, eval = FALSE}
for (i in 1:4){
  print(i)
}
...
Links here:
https://github.ubc.ca/mds-2025-26/DSCI_521_lab3_group08/blob/master/question_7.html
https://github.ubc.ca/mds-2025-26/DSCI_521_lab3_group08/blob/master/question_7.pdf
...
Links here:   
https://github.ubc.ca/mds-2025-26/DSCI_521_lab3_group08/blob/master/question_8.Rmd
https://github.ubc.ca/mds-2025-26/DSCI_521_lab3_group08/blob/master/question_8.html
...
The file preview in github as the visual markdown presentation of the file.The 
headers and titles are rendered in their visual format, whereas the rmd and
html files show their coding, source in the preview on github. 
...
https://yauchungki1305.github.io/lab3_presentation/
...
Reproducible tools like R Markdown and Jupyter use code and data in a single document. This could be useful if multiple people are working or viewing the document. Anyone can open the presentation or report in R Markdown or Jupyter and render or knit the document, and the final result should be the same for each user. Each time someone runs the document, the results are recalculated and reproduced, making it easy to trust the data and work. Non-reproducible tools like Word, Powerpoint and Keynote are good for user friendliness. When the user edits the document, they are directly editing what the final version will look like. They are highly flexible, but lack the integration with data and code, making the results un-reproducible and hard to verify. 
